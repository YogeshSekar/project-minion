  ...
  return <div className="flex h-full min-h-0 min-w-0 flex-col bg-slate-100 text-slate-900">
    <div className="relative z-30 flex h-16 flex-none items-center border-b border-slate-200 bg-white">
      <div className="flex h-16 w-72 flex-none items-center gap-2 border-r border-slate-200 px-3">
        <button onClick={() => setProjectModal({ open: true, mode: 'create', project: null })} className="inline-flex h-9 flex-1 items-center justify-center gap-2 rounded-full bg-accent-solid px-4 text-sm font-semibold text-accent-foreground hover:bg-accent-solid-hover"><Plus className="h-4 w-4" /> New project</button>
      </div>
      <div className="flex min-w-0 flex-1 items-center justify-between gap-4 px-5">
        {selected ? <><div className="flex min-w-0 items-center gap-2"><span className={`h-2.5 w-2.5 flex-none rounded-full ${(STATUS[selected.status] || STATUS.planning)[1]}`} /><h1 className="truncate text-lg font-semibold tracking-tight">{selected.title}</h1><span className="hidden rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-medium text-slate-500 sm:inline">{(STATUS[selected.status] || STATUS.planning)[0]}</span><button onClick={() => setProjectModal({ open: true, mode: 'edit', project: selected })} className="grid h-8 w-8 flex-none place-items-center rounded-full border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-accent-text" title="Edit project"><Edit3 className="h-3.5 w-3.5" /></button></div><div className="flex flex-none items-center gap-2">{selected.status !== 'completed' ? <button onClick={() => updateProject({ ...selected, status: 'completed' })} className="inline-flex h-9 items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 text-sm font-semibold text-emerald-700"><CheckCircle2 className="h-4 w-4" /> Complete</button> : <button onClick={() => updateProject({ ...selected, status: 'in_progress' })} className="h-9 rounded-full border border-accent-border bg-accent-surface px-3 text-sm font-semibold text-accent-text">Reopen</button>}<button onClick={() => askDelete('project', selected, async () => { await deleteProject(selected.id); setSelectedId(null) })} className="grid h-9 w-9 place-items-center rounded-full border border-slate-200 text-slate-500 hover:bg-rose-50 hover:text-rose-600" title="Delete project"><Trash2 className="h-4 w-4" /></button></div></> : <span className="text-sm text-slate-500">Select or create a project</span>}
      </div>
    </div>
      ...
      <aside className="min-h-0 overflow-y-auto border-b border-slate-200 bg-white lg:border-b-0 lg:border-r" aria-label="Projects">
        ...
        {loading ? <p className="p-5 text-center text-sm text-slate-500">Loading projects…</p> : filteredProjects.length ? <div className="divide-y divide-slate-100">{filteredProjects.map(project => {
          ...
          const projectProgress = ownTasks.length ? Math.round(done / ownTasks.length * 100) : Number(project.progress || 0)
            return <button key={project.id} onClick={() => setSelectedId(project.id)} className={`relative flex w-full flex-col gap-2 px-4 py-3 text-left transition-colors ${selectedId === project.id ? 'bg-accent-surface/70' : 'hover:bg-slate-50'}`}>{selectedId === project.id && <span className="absolute inset-y-0 left-0 w-0.5 bg-accent-solid" />}<span className={`line-clamp-2 text-sm font-semibold ${selectedId === project.id ? 'text-accent-text' : 'text-slate-800'}`}>{project.title}</span><span className="flex items-center gap-2 text-xs text-slate-500"><span className={`h-2 w-2 rounded-full ${(STATUS[project.status] || STATUS.planning)[1]}`} />{(STATUS[project.status] || STATUS.planning)[0]}<span className="ml-auto">{projectProgress}%</span></span><span className="h-1 overflow-hidden rounded-full bg-slate-100"><span className="block h-full rounded-full bg-accent-solid" style={{ width: `${Math.min(projectProgress, 100)}%` }} /></span></button>
      ...
      <main className="flex min-h-0 min-w-0 flex-col">
        {!selected ? <div className="flex flex-1 flex-col items-center justify-center text-center"><FolderKanban className="h-8 w-8 text-slate-300" /><p className="mt-3 text-sm font-semibold text-slate-700">No project selected</p><p className="mt-1 text-xs text-slate-500">Choose a project or create a new one.</p></div> : <>
          <div className="flex flex-none flex-wrap items-center gap-x-7 gap-y-2 border-b border-slate-200 bg-white px-5 py-3 text-sm">
              <span className="inline-flex items-center gap-2 text-slate-600"><Flag className="h-4 w-4 text-accent-text" />{(PRIORITY[selected.priority] || PRIORITY.medium)[0]}</span>
              <span className={`inline-flex items-center gap-2 ${selected.deadline && selected.status !== 'completed' && isOverdue(selected.deadline) ? 'font-semibold text-rose-600' : 'text-slate-600'}`}><CalendarDays className="h-4 w-4 text-accent-text" />{selected.deadline ? formatDate(selected.deadline) : 'No deadline'}</span>
              <span className="inline-flex items-center gap-2 text-slate-600"><Clock3 className="h-4 w-4 text-accent-text" />{projectTasks.length - completedTasks} open tasks</span>
              <div className="ml-auto flex w-48 items-center gap-2"><span className="h-1.5 flex-1 overflow-hidden rounded-full bg-slate-100"><span className="block h-full rounded-full bg-accent-solid" style={{ width: `${Math.min(progress, 100)}%` }} /></span><span className="text-xs font-semibold text-accent-text">{progress}%</span></div>
          </div>
