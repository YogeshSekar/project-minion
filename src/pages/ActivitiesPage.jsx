      ...
      <div className="flex h-16 min-w-0 flex-1 items-center gap-2 overflow-x-auto px-3 no-scrollbar">
        <div ref={rangeRailRef} className="relative flex h-10 items-center gap-1 rounded-full border border-slate-200 bg-slate-50 p-1">
          <span aria-hidden="true" className={`pointer-events-none absolute inset-y-1 left-0 z-0 rounded-full border border-slate-200 bg-white transition-[transform,width,opacity] duration-300 ease-out ${rangeIndicator.visible ? 'opacity-100' : 'opacity-0'}`} style={{ width: rangeIndicator.width, transform: `translateX(${rangeIndicator.left}px)` }} />
              {RANGE_OPTIONS.map(([id, label]) => <button ref={element => { if (element) rangeRefs.current.set(id, element); else rangeRefs.current.delete(id) }} key={id} onClick={() => setRange(id)} className={`relative z-10 h-8 flex-none rounded-full px-3.5 text-sm font-medium transition-colors duration-300 ${range === id ? 'text-accent-text' : 'text-slate-700 hover:text-slate-900'}`}>{label}</button>)}
        </div>
    ...
    <div className="flex min-h-0 flex-1">
          ...
          <div className="relative pl-14">
            ...
            <div className="space-y-1.5 border-l border-slate-200 pl-3">
              {dayTimeline.map((row, index) => {
                ...
                if (row.type === 'future') return <div key={index} className="rounded-lg bg-slate-50 px-3 py-2 text-[10px] text-slate-300 dark:bg-slate-800 dark:text-slate-500" style={{ minHeight: proportionalHeight }}>Future time</div>
                if (row.type === 'gap') return <div key={index} className="relative flex flex-col justify-center rounded-lg border border-dashed border-slate-200 bg-slate-50/80 px-3 py-2 dark:border-slate-700 dark:bg-slate-800/80" style={{ minHeight: proportionalHeight }}><span className="absolute -left-[18px] top-1/2 h-2 w-2 -translate-y-1/2 rounded-full border border-slate-300 bg-white dark:border-slate-600 dark:bg-slate-900" /><p className="text-[10px] font-medium tabular-nums text-slate-500 dark:text-slate-300">{timeLabel(row.start)} – {timeLabel(row.end)}</p><p className="mt-0.5 text-[10px] text-slate-400 dark:text-slate-500">Untracked · {durationLabel(row.end - row.start)}</p></div>
                const item = row.item
                      return <div key={`${item.id}-${index}`} className={`relative flex flex-col justify-center rounded-lg border px-3 py-2 ${item.status === 'running' ? 'border-emerald-200 bg-emerald-50' : item.activity_type === 'meeting' ? 'border-amber-200 bg-amber-50' : 'border-accent-border bg-accent-surface'}`} style={{ minHeight: proportionalHeight }}><span className={`absolute -left-[18px] top-1/2 h-2.5 w-2.5 -translate-y-1/2 rounded-full ring-2 ring-white ${item.status === 'running' ? 'animate-pulse bg-emerald-500' : item.activity_type === 'meeting' ? 'bg-amber-400' : 'bg-accent-solid'}`} /><p className="truncate text-xs font-semibold text-slate-800">{item.title}</p><p className="mt-1 text-[10px] font-medium tabular-nums text-slate-500">{timeLabel(row.start)} – {timeLabel(row.end)}</p><p className="mt-0.5 text-[10px] text-slate-400">{item.status === 'running' ? 'Running' : durationLabel(row.end - row.start)}</p></div>
              })}
            </div>
      ...
      <main className="grid min-w-0 flex-1 grid-cols-2 gap-3 overflow-y-auto bg-slate-100 p-3">
        <section className="min-w-0 space-y-3">
          ...
          <StatCard icon={Target} label="Tasks worked" value={analytics?.tasks?.length || 0} detail={`${durationLabel(taskSeconds * 1000)} spent on tasks`} tone="bg-accent-surface text-accent-text" />
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5">
            <div className="mb-5 flex items-center gap-2"><BriefcaseBusiness className="h-4 w-4 text-accent-text" /><h3 className="text-sm font-semibold text-slate-900">Time spent by project</h3></div>
            {(analytics?.projects || []).length ? <div className="space-y-4">{analytics.projects.map(project => { const percentage = analytics.total_seconds ? Math.round(project.total_seconds / analytics.total_seconds * 100) : 0; return <div key={project.key}><div className="flex items-center gap-3"><span className="min-w-0 flex-1 truncate text-sm font-medium text-slate-700">{project.label}</span><span className="text-xs text-slate-400">{percentage}%</span><span className="w-16 text-right text-xs font-semibold tabular-nums text-slate-600">{durationLabel(project.total_seconds * 1000)}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-accent-solid" style={{ width: `${Math.max(percentage, project.total_seconds ? 2 : 0)}%` }} /></div></div>})}</div> : <p className="py-8 text-center text-xs text-slate-400">No project time in this period</p>}
          </section>
        </section>
