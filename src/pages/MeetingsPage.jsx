    ...
    <div className="flex h-full min-h-0 min-w-0 flex-col bg-slate-100 text-slate-900">
      <div className="relative z-20 flex h-16 flex-none items-center border-b border-slate-200 bg-white">
        ...
        <div className="flex min-w-0 flex-1 items-center justify-between gap-4 px-5">
          ...
          <div className="relative w-44 flex-none"><FolderKanban className="pointer-events-none absolute left-3 top-2.5 z-10 h-4 w-4 text-accent-text" /><Select value={selectedProjectId ?? ''} disabled={busy || !outlookId} onChange={value => changeProject(value ? Number(value) : null)} options={[{ value: '', label: 'No project' }, ...projects.map(project => ({ value: project.id, label: project.title }))]} ariaLabel="Meeting project" triggerClassName="border-accent-border bg-accent-surface pl-9 font-semibold text-accent-text hover:border-accent-focus hover:bg-accent-surface-hover" menuClassName="right-0 left-auto w-56" /></div>
          {meetingUrl && !isCanceled(meeting) && <a href={meetingUrl} target="_blank" rel="noopener noreferrer" className="inline-flex h-9 items-center justify-center gap-2 rounded-full border border-accent-solid bg-accent-solid px-4 text-sm font-semibold text-accent-foreground transition-colors duration-200 hover:border-accent-solid-hover hover:bg-accent-solid-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-focus"><Video className="h-4 w-4" /> Join</a>}
          ...
          {editingUrl && <div className="absolute right-0 top-11 z-40 flex w-80 gap-2 rounded-xl border border-slate-200 bg-white p-3 shadow-xl"><input value={urlDraft} onChange={event => setUrlDraft(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') saveUrl(); if (event.key === 'Escape') setEditingUrl(false) }} placeholder="https://…" className="h-9 min-w-0 flex-1 rounded-full border border-slate-200 px-3 text-xs outline-none focus:border-accent-focus" autoFocus /><button onClick={saveUrl} disabled={busy || !urlDraft.trim()} className="grid h-9 w-9 flex-none place-items-center rounded-full bg-accent-solid text-accent-foreground disabled:opacity-50" aria-label="Save meeting link"><Check className="h-4 w-4" /></button><button onClick={() => setEditingUrl(false)} className="grid h-9 w-9 flex-none place-items-center rounded-full border border-slate-200" aria-label="Cancel"><X className="h-4 w-4" /></button></div>}</div>
            </div>
          </>}
        </div>
      </div>
      ...
      {loading ? <div className="flex flex-1 items-center justify-center text-sm text-slate-500">Loading Outlook meetings…</div>
        : error ? <div className="flex flex-1 flex-col items-center justify-center gap-3 text-sm text-slate-600"><p>{error}</p><button onClick={() => setReloadKey(value => value + 1)} className="rounded-full bg-accent-solid px-4 py-2 font-semibold text-accent-foreground">Retry</button></div>
          : <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[288px_minmax(0,1fr)]">
              <aside className="min-h-0 overflow-y-auto border-b border-slate-200 bg-white lg:border-b-0 lg:border-r" aria-label="Meetings for selected day">
                ...
                return <button key={`${meetingKey(item)}-${index}`} type="button" onClick={() => { setSelectedIndex(index); setActionError(''); setEditingUrl(false) }} aria-current={selectedIndex === index ? 'true' : undefined} className={`relative flex w-full flex-col gap-1 px-4 py-3 text-left transition-colors ${selectedIndex === index ? 'bg-accent-surface/70' : 'hover:bg-slate-50'} ${isCanceled(item) ? 'opacity-65' : ''}`}>
                  {selectedIndex === index && <span className="absolute inset-y-0 left-0 w-0.5 bg-accent-solid" />}
                  <span className="flex w-full items-start gap-2"><span className={`min-w-0 flex-1 line-clamp-2 text-sm font-semibold leading-snug ${selectedIndex === index ? 'text-accent-text' : 'text-slate-800'}`}>{meetingTitle(item)}</span>{itemIsRunning && <span className="mt-1 h-2 w-2 flex-none animate-pulse rounded-full bg-emerald-500" title="Tracking" />}{isCanceled(item) && <span className="flex-none rounded-full border border-slate-200 bg-slate-50 px-1.5 py-0.5 text-[10px] font-medium text-slate-500">Canceled</span>}</span>
            ...
            <div className="flex h-16 flex-none items-center gap-8 border-b border-slate-200 bg-white px-5 text-sm font-medium text-slate-700"><span className="inline-flex flex-none items-center gap-2"><Clock3 className="h-4 w-4 text-accent-text" />{timeLabel(meeting.start)} – {timeLabel(meeting.end)}</span><span className="inline-flex min-w-0 items-center gap-2"><MapPin className="h-4 w-4 flex-none text-accent-text" /><span className="truncate" title={meeting.location || ''}>{meeting.location || 'No location'}</span></span></div>
