      ...
      >
        ...
        <div className="flex h-14 flex-none items-center" data-tauri-drag-region="false">
          <div className="flex h-14 w-16 flex-none items-center justify-center bg-slate-900">
            <CheckSquare className="h-6 w-6 text-indigo-300" strokeWidth={1.8} aria-hidden="true" />
          </div>
        ...
        <div className="flex-1 flex items-center justify-center px-4" data-tauri-drag-region="false">
          <div ref={searchContainerRef} className="relative w-full max-w-lg">
            <div className={`
              ...
              focus-within:bg-white focus-within:border-indigo-300 focus-within:ring-2 focus-within:ring-indigo-100
              transition-all duration-200
              ${isSearchFocused ? 'bg-white border-indigo-300 ring-2 ring-indigo-100' : ''}
            `}>
              <Search className={`h-4 w-4 flex-shrink-0 transition-colors ${isSearchFocused ? 'text-indigo-600' : 'text-slate-400'}`} />
              <input
                ref={searchInputRef}
                type="text"
                value={searchQuery}
            ...
            {isSearchFocused && (
              <div className="absolute left-0 right-0 z-[70] mt-2 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_18px_50px_rgba(15,23,42,0.18)]">
                ...
                {!normalizedSearchQuery ? (
                  ...
                  <div className="max-h-[420px] overflow-y-auto p-2">
                    <div className="flex items-center justify-between gap-3 px-2 py-1.5">
                      ...
                      <div className="flex items-center gap-2">
                        <label className="flex cursor-pointer items-center gap-1.5 text-[11px] font-medium text-slate-500">
                          <span>Completed</span>
                          <button type="button" role="switch" aria-checked={showCompletedSearchResults} onClick={toggleCompletedSearchResults} className={`relative inline-flex h-4 w-7 rounded-full transition-colors ${showCompletedSearchResults ? 'bg-indigo-600' : 'bg-slate-300'}`}>
                    ...
                    {taskSearchResults.map(task => {
                      ...
                      return <button key={task.id} type="button" onClick={() => openTaskResult(task)} className="group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left transition-colors hover:bg-indigo-50 focus:bg-indigo-50 focus:outline-none">
                        <span className={`grid h-8 w-8 flex-none place-items-center rounded-lg ${task.status === 'completed' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-500 group-hover:bg-white group-hover:text-indigo-600'}`}><ListTodo className="h-4 w-4" /></span>
                ...
                <div className="flex items-center justify-between border-t border-slate-100 bg-slate-50/70 px-4 py-2 text-[11px] text-slate-400">
                  ...
                  {searchQuery && <button type="button" onClick={() => { setSearchQuery(''); searchInputRef.current?.focus() }} className="inline-flex items-center gap-1 font-semibold text-slate-500 hover:text-indigo-700"><XCircle className="h-3.5 w-3.5" />Clear query</button>}
                </div>
              </div>
            )}
          </div>
        ...
        <div className="ml-auto flex items-center gap-3 pr-3" data-tauri-drag-region="false">
          ...
          <div className="relative" id="timer-dropdown">
            <div className={`flex h-9 items-center overflow-hidden rounded-full border bg-white transition-colors ${isTimerRunning ? 'border-emerald-200 bg-emerald-50/70' : trackedTask ? 'border-indigo-200' : 'border-slate-200'}`}>
              <button type="button" onClick={() => setIsTimerOpen(!isTimerOpen)} className="flex h-full min-w-0 items-center gap-2 px-3 text-left hover:bg-slate-50/80" title="Open task timer">
                <span className={`relative grid h-6 w-6 flex-none place-items-center rounded-full ${isTimerRunning ? 'bg-emerald-100 text-emerald-700' : 'bg-indigo-50 text-indigo-600'}`}><Clock3 className="h-3.5 w-3.5" />{isTimerRunning && <span className="absolute right-0 top-0 h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-500" />}</span>
              ...
              <button type="button" onClick={() => runTimerAction(() => isTimerRunning ? pauseTimer() : startTimer())} disabled={timerBusy || (!isTimerRunning && !trackedTask)} className={`grid h-full w-9 flex-none place-items-center transition-colors disabled:cursor-not-allowed disabled:opacity-35 ${isTimerRunning ? 'text-emerald-700 hover:bg-emerald-100' : 'text-indigo-600 hover:bg-indigo-50'}`} title={isTimerRunning ? 'Pause tracking' : trackedTask ? 'Resume tracking' : 'Choose a task to start'}>{timerBusy ? <Loader2 className="h-4 w-4 animate-spin" /> : isTimerRunning ? <Pause className="h-4 w-4 fill-current" /> : <Play className="h-4 w-4 fill-current" />}</button>
            </div>
